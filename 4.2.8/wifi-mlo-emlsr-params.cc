 ...
 *
 * Evaluates how key EMLSR parameters affect DL throughput and latency
 * under a fixed moderate load (normalizedLoad controlled by runner).
 *
 * Parameters varied (one at a time, others at baseline):
 *   - EmlsrTransitionDelay: 0, 16, 32, 64, 128, 256 µs
 *   - EmlsrPaddingDelay:    0, 32, 64, 128, 256 µs
 *   - TransitionTimeout:    128, 256, 512, 1024, 2048, 4096, 8192, 16384 µs
 *   - AuxPhyChannelWidth:   20, 40, 80 MHz
 *   - PutAuxPhyToSleep:     true / false
 *   - AuxPhyTxCapable:      true / false
 *   - SwitchAuxPhy:         true / false
 *
 * Single BSS: 1 AP + nStas STAs, 2 links (5 GHz ch42 + 6 GHz ch7, 80 MHz each)
 *
 * PHY: EHT 802.11be, MCS 11, 2 SS, GI 800 ns
 * A-MPDU: 1024 MPDUs
 * Traffic: constant-rate DL (AP → STA), normalized load
 * AP–STA distance: 5 m
 *
...
NS_LOG_COMPONENT_DEFINE("WifiMloEmlsrParams");
...
main(int argc, char* argv[])
{
    ...
    uint32_t    nStas              = 1;
    double      normalizedLoad     = 0.7;

    // EMLSR parameters (all sweepable)
    uint32_t    transitionDelayUs  = 0;     // 0,16,32,64,128,256
    uint32_t    paddingDelayUs     = 0;     // 0,32,64,128,256
    uint32_t    transitionTimeoutUs= 128;   // 128..65536 (powers of 2 from 2^7)
    uint32_t    auxPhyWidth        = 80;    // 20,40,80
    ...
    cmd.AddValue("transitionDelayUs",   "EMLSR transition delay [µs]",          transitionDelayUs);
    cmd.AddValue("paddingDelayUs",      "EMLSR padding delay [µs]",             paddingDelayUs);
    cmd.AddValue("transitionTimeoutUs", "AP transition timeout [µs]",           transitionTimeoutUs);
    cmd.AddValue("auxPhyWidth",         "Aux PHY channel width [MHz]",          auxPhyWidth);
    cmd.AddValue("putAuxPhyToSleep",    "Put aux PHY to sleep",                 putAuxPhyToSleep);
    cmd.AddValue("auxPhyTxCapable",     "Aux PHY TX capable",                   auxPhyTxCapable);
    cmd.AddValue("switchAuxPhy",        "Switch aux PHY on main link change",   switchAuxPhy);
    cmd.Parse(argc, argv);
    ...
    std::cout << "=== EMLSR Parameter Variation ===\n"
              << "nStas: " << nStas
              << "  load: " << normalizedLoad
              << "  transDelay: " << transitionDelayUs << " us"
              << "  padDelay: " << paddingDelayUs << " us"
              << "  timeout: " << transitionTimeoutUs << " us"
              << "  auxWidth: " << auxPhyWidth
              << "  sleep: " << putAuxPhyToSleep
              << "  txCap: " << auxPhyTxCapable
              << "  switchAux: " << switchAuxPhy
              << "\n\n";
    ...
    Ptr<MultiModelSpectrumChannel> specCh6 = makeChannel();
    ...
    // -----------------------------------------------------------------------
    //  PHY (2 links: 5 GHz + 6 GHz, both 80 MHz)
    // -----------------------------------------------------------------------
    ...
    wifi.ConfigEhtOptions("TransitionTimeout", TimeValue(MicroSeconds(transitionTimeoutUs)));

    // -----------------------------------------------------------------------
    //  MAC
    // -----------------------------------------------------------------------
    WifiMacHelper wifiMac;
    Ssid ssid = Ssid("emlsr-params");
    ...
    wifiMac.SetEmlsrManager("ns3::DefaultEmlsrManager",
                            ...
                            "SwitchAuxPhy",          BooleanValue(switchAuxPhy),
                            "AuxPhyChannelWidth",    UintegerValue(auxPhyWidth),
                            "AuxPhyMaxModClass",     StringValue("EHT"),
                            "AuxPhyTxCapable",       BooleanValue(auxPhyTxCapable),
                            "PutAuxPhyToSleep",      BooleanValue(putAuxPhyToSleep),
                            "EmlsrPaddingDelay",     TimeValue(MicroSeconds(paddingDelayUs)),
                            "EmlsrTransitionDelay",  TimeValue(MicroSeconds(transitionDelayUs)));
    ...
    // omitted. It calls WifiNetDevice::GetChannel() under the hood, which
    // ns-3 forbids on multi-link devices (EMLSR). Single BSS (AP+STAs on one
    // /24) needs no IP routing — ARP + L2 handles everything.

    // -----------------------------------------------------------------------
    //  Traffic
    //  Reference peak (80 MHz, 1 STA, nMpdus=1024): 1114 Mb/s
    // -----------------------------------------------------------------------
